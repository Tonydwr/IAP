 ...
 private SwipeDir AnalyzeSwipe(Swipe swipe)
{
    ...
    if (Vector3.Distance(Camera.main.ScreenToWorldPoint(new Vector3(swipe.end.x, swipe.end.y, 2f)), b) < 10f)
    {
...
void HandleSwipe(SwipeDir dir){
	...
	{
		m_IsRunning = true;
		// character.rigidbody.velocity = Vector3.forward;
	}
	...
	protected void Update ()
    {
        ...
        if (Input.touchCount == 1)
        {
			if(m_IsSwiping)
			{
				...
				if(diff.magnitude > 0.02f) //we set the swip distance to trigger movement to 1% of the screen width
				{
